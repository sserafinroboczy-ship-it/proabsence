...
export const sendLimitWarningEmail = async (recipients: string[], data: LimitWarningData) => {
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.log('⚠️ SMTP nie skonfigurowany - pomijam wysyłkę maila');
    return false;
  }
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Zbliżanie się do limitu godzin - ProAbsence</title>
</head>
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f7fa;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
    <tr>
            ...
            <td style="background: linear-gradient(135deg, #1e3a5f 0%, #2d5286 100%); padding: 30px 40px; border-radius: 12px 12px 0 0;">
              <table role="presentation" style="width: 100%;">
                <tr>
                  ...
                  <td align="right">
                    <div style="background-color: #f59e0b; color: white; padding: 8px 16px; border-radius: 20px; font-size: 12px; font-weight: 600;">
                      ⚠️ OSTRZEŻENIE
                    </div>
                  </td>
                </tr>
          ...
          <tr>
            <td style="padding: 40px;">
              <h2 style="margin: 0 0 20px 0; color: #d97706; font-size: 22px; font-weight: 600;">
                ⏳ Zbliżanie się do limitu godzin
              </h2>
              
              <p style="margin: 0 0 25px 0; color: #64748b; font-size: 15px; line-height: 1.6;">
                Pracownik zbliża się do miesięcznego limitu godzin. Pozostało mniej niż <strong>20 godzin</strong>. Poniżej szczegóły:
              </p>

              <!-- Karta pracownika -->
              <table role="presentation" style="width: 100%; border-collapse: collapse; background-color: #fffbeb; border-radius: 10px; border: 1px solid #fcd34d; margin-bottom: 25px;">
                <tr>
                  <td style="padding: 25px;">
                    <table role="presentation" style="width: 100%;">
                      <tr>
                        <td style="padding-bottom: 15px; border-bottom: 1px solid #fcd34d;">
                          <span style="color: #92400e; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">Dane pracownika</span>
                        </td>
                      </tr>
                      <tr>
                        <td style="padding-top: 15px;">
              ...
              <table role="presentation" style="width: 100%; border-collapse: collapse; margin-bottom: 25px;">
                <tr>
                  ...
                  <td style="width: 33%; padding: 15px; background-color: #fef9c3; border-radius: 0 10px 10px 0; text-align: center;">
                    <div style="color: #a16207; font-size: 12px; font-weight: 500; margin-bottom: 5px;">POZOSTAŁO</div>
                    <div style="color: #a16207; font-size: 24px; font-weight: 700;">${data.hoursRemaining}h</div>
                  </td>
                </tr>
              </table>
  ...
  const textContent = `
PROABSENCE - ZBLIŻANIE SIĘ DO LIMITU GODZIN
=============================================

⚠️ OSTRZEŻENIE: Pracownik zbliża się do miesięcznego limitu godzin (pozostało < 20h).
...
- Limit: ${data.hoursLimit}h
- Wykorzystano: ${data.hoursUsed}h
- Pozostało: ${data.hoursRemaining}h
...
Ta wiadomość została wygenerowana automatycznie przez system ProAbsence.
  ...
  try {
    await transporter.sendMail({
      from: `"${fromName}" <${fromEmail}>`,
      to: recipients.join(', '),
      subject: `⚠️ [ProAbsence] Zbliżanie się do limitu godzin - ${data.firstName} ${data.lastName} (pozostało ${data.hoursRemaining}h)`,
      text: textContent,
      html: htmlContent,
    });
    console.log(`✅ Mail warning wysłany do: ${recipients.join(', ')}`);
    return true;
  } catch (error) {
    console.error('❌ Błąd wysyłki maila warning:', error);
    return false;
  }
};
